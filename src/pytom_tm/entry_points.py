...
import logging
import numpy as np
from pytom_tm.io import (LargerThanZero, write_mrc, read_mrc_meta_data, read_mrc, CheckFileExists, ParseLogging,
        CheckDirExists, ParseSearch, ParseTiltAngles, ParseDoseFile, ParseDefocusFile, BetweenZeroAndOne)
...
def create_template(argv=None):
    ...
    if args.input_voxel_size_angstrom is not None:
        if round(args.input_voxel_size_angstrom, 3) != round(input_meta_data['voxel_size'], 3):
...
def estimate_roc(argv=None):
    ...
    parser = argparse.ArgumentParser(description='Estimate ROC curve from TMJob file. -- Marten Chaillet (@McHaillet)')
    ...
    parser.add_argument('--bins', type=int, required=False, action=LargerThanZero, default=20,
                        help='Number of bins for the histogram to fit Gaussians on.')
    parser.add_argument('--gaussian-peak', type=int, required=False, action=LargerThanZero,
                        help='Expected index of the histogram peak of the Gaussian fitted to the particle population.')
    parser.add_argument('--force-peak', action='store_true', default=False, required=False,
                        help='Force the particle peak to the provided peak index.')
    parser.add_argument('--crop-plot', action='store_true', default=False, required=False,
    ...
    plist_quality_gaussian_fit(
        ...
        output_figure_name=(
            None if args.show_plot else
            template_matching_job.output_dir.joinpath(f'{template_matching_job.tomo_id}_roc.svg')
        ),
