...
This means we sample the map on a 1.724A grid (which corresponds to the tilt-series' pixel size of this dataset) and at a resolution of 5A. `#1/L?` selects only chains starting with L from loaded model 1, i.e. chains that belong to the large subunit. Afterwards save the generated potential as an .mrc file: `6qzp_60S.mrc`. (It can also be downloaded [here](data/6qzp_60S.mrc))
...
Secondly, we need a mask with the same box size as the template and a radius that fully encompasses our ribosome structure. The diameter of a ribosome is roughly 300A, which means we need at least (300A / 13.79A =) 22 pixels to cover the ribosome. With some overhang we extend it to 24 pixels diameter, and therefore set the mask radius (`--radius`) to 12 pixels. The mask will also need a smooth fall-off to prevent aliasing artifacts, which we set with `--sigma`:
...
| ---- |
| <img src="images/0_template_mask.png" width="400"> |
...
| ---- |
| <img src="images/1_tomo200528_100_slice69_comb.png" width="800"> |
...
| ---- |
| <img src="images/1_tomo200528_100_roc_80S.svg" width="800"> |
...
to 35A to include more high-resolution information in the template matching. This 
results in an angular sampling ~6.5 degrees. We 
could also entirely remove it, but this would increase the sampling to 5.3 degrees 
...
|--------------------------------------------------------------------------------------------------|
| <img src="images/2_tomo200528_100_slice69_simple.png" width="800">                               |
...
|----------------------------------------------------------------|
| <img src="images/2_tomo200528_100_roc_simple.svg" width="800"> |
...
To improve the results even further, the angular sampling needs to be increased. For 
part 2 of this tutorial, we set the low-pass filter to 35 A. This means template 
matching is now including resolution up to 1 / 35 $A^{-1}$, 
